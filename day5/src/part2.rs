...
impl Matrix {
    ...
    fn insert_diagonal_line(&mut self, col_s: usize, row_s: usize, col_e: usize, row_e: usize) {
        let (mut rfrom, mut rto, mut cfrom, mut cto) = (row_s, row_e, col_s, col_e);
    ...
    fn insert_line(&mut self, line: &str) -> Result<()> {
        ...
        let drow = (row_s as i64 - row_e as i64).abs();
        let dcol = (col_s as i64 - col_e as i64).abs();
        ...
        if row_s == row_e {
            ...
            self.ensure_size(max(row_s, row_e), max(col_s, col_e));
            self.insert_diagonal_line(col_s, row_s, col_e, row_e);
        }
        Ok(())
    }
